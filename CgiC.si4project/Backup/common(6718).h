...
typedef struct Conf
{
    ...
    char FS_Port[10];               //FS-TCP端口号
    char FS_Udp_Port[10];           //FS-UDP端口号
    char EPS_IP[16];            	//EPS的ip地址
    char EPS_Port[10];              //EPS端口号      
    char Time_Out[10];              //心跳超时时间
    char Time_Reconnect[10];        //重连时间间隔
    char OPT_IS_Online[10];         //主板没有回复转接板的时间，断开FS和转接板的链接
    
    char My_IP[16];             	//本地ip地址    
    char My_FS_Port[10];            //本地server 与FS端口号
    char My_EPS_Port[10];           //本地server 与EPS端口号
    char My_Mask[16];           	//本地掩码
    char My_Gateway[16];        	//本地网关
    char My_Broadcast_IP[16];   	//本地广播地址
    char My_Broadcast_Port[10];     //本地广播地址端口       
    char Led_ON[2];               	//心跳灯控制  
